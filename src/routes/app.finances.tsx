...
import { ResponsiveContainer, AreaChart, Area, Tooltip, PieChart, Pie, Cell, XAxis } from "recharts";
  ...
  return (
    <div className="px-4 pt-5 space-y-4 animate-rise">
      <header className="flex items-center justify-between pt-2">
        <div>
          <p className="text-[11px] text-muted-foreground uppercase tracking-wider font-medium">Portfolio</p>
      ...
      <section>
        <SectionHeader
          title="Holdings"
          action={<span className="text-[11px] text-muted-foreground">{investments.length} positions</span>}
        />
          ...
          <EmptyState
            ...
            action={
              <div className="flex flex-col sm:flex-row gap-2 items-center">
                <Button size="sm" onClick={() => setOpen(true)}><Plus className="size-3.5 mr-1" /> Add holding</Button>
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-3 min-w-0">
                    <div className="size-10 rounded-xl grid place-items-center shrink-0" style={{ background: `${meta.color}22`, color: meta.color }}>
                      <Icon className="size-4" />
                    </div>
                    <div className="min-w-0">
                      <div className="text-sm font-semibold truncate flex items-center gap-1.5">
                        {inv.ticker && <span className="font-mono text-[11px] uppercase opacity-60">{inv.ticker}</span>}
                        {inv.name}
                      </div>
                  ...
                  <div className="text-right shrink-0 ml-3 flex items-center gap-2">
                    ...
                    <button
                      onClick={(e) => { e.preventDefault(); deleteInvestment.mutate(inv.id); }}
      ...
      {investments.length > 0 && (
        <div className="card-soft p-4 bg-foreground text-background relative overflow-hidden">
          ...
          <div className="relative flex gap-3">
            ...
            <div>
              <p className="text-[10px] uppercase tracking-wider opacity-60 font-medium">Portfolio insight</p>
...
function InvestmentDialog({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  function save() {
    ...
    addInvestment.mutate(
      ...
      {
        onSuccess: () => {
          setTicker(""); setName(""); setQuantity(""); setAvgCost(""); setCurrentPrice("");
          onClose();
        },
      },
    );
        ...
        <div className="space-y-3">
            ...
            {(["stock", "etf", "crypto", "savings", "other"] as const).map((t) => (
              <button
                ...
                className={`py-2 text-[11px] font-medium rounded-lg border transition capitalize ${
                  type === t ? "bg-foreground text-background border-foreground" : "border-border bg-surface text-muted-foreground"
                }`}
              >{t}</button>
            ))}
          </div>
          <div className="grid grid-cols-3 gap-3">
            <div className="space-y-1.5 col-span-1">
              <Label>Ticker</Label>
              <Input value={ticker} onChange={(e) => setTicker(e.target.value.toUpperCase())} placeholder="VWCE" />
            </div>
            <div className="space-y-1.5 col-span-2">
              <Label>Name</Label>
              <Input value={name} onChange={(e) => setName(e.target.value)} placeholder="Vanguard FTSE All-World" />
            </div>
          </div>
          <div className="grid grid-cols-3 gap-3">
            ...
            <div className="space-y-1.5">
              ...
              <Input type="number" value={avgCost} onChange={(e) => setAvgCost(e.target.value)} placeholder="100" />
            </div>
            <div className="space-y-1.5">
              <Label>Price now</Label>
              <Input type="number" value={currentPrice} onChange={(e) => setCurrentPrice(e.target.value)} placeholder="110" />
            </div>
          </div>
