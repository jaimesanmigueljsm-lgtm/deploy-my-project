...
export const pinStore = {
  read:  (uid: string): string | null => {
    if (!safe) return null;
    try { return localStorage.getItem(PIN_KEY(uid)); } catch { return null; }
  },
  write: (uid: string, hash: string) => {
    if (!safe) return;
    try { localStorage.setItem(PIN_KEY(uid), hash); } catch {}
  },
  clear: (uid: string) => {
    if (!safe) return;
    try { localStorage.removeItem(PIN_KEY(uid)); } catch {}
  },
};
...
export const promptStore = {
  wasShown: (uid: string): boolean => {
    ...
    try { return !!localStorage.getItem(PROMPTED_KEY(uid)); } catch { return false; }
  },
  markShown: (uid: string) => {
    if (!safe) return;
    try { localStorage.setItem(PROMPTED_KEY(uid), "1"); } catch {}
  },
};
